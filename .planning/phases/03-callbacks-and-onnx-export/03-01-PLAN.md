...
3. Create `src/classifier_training/callbacks/onnx_export.py` — adapted from sibling `onnx_export.py`:
   ...
   - After ONNX export, also write `labels_mapping.json` sidecar by calling `datamodule.save_labels_mapping(Path(self.output_dir) / "labels_mapping.json")` (if datamodule available on trainer). Note: `save_labels_mapping` expects a FULL FILE PATH, not a directory — it calls `open(save_path, "w")` directly.
