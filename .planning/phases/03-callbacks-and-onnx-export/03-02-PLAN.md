...
must_haves:
  truths:
    ...
    - "ModelInfoCallback computes parameter count, model size, and writes labels_mapping.json sidecar (FLOPs deferred — not included per research decision to avoid fvcore dependency)"
...
3. Create `src/classifier_training/callbacks/model_info.py` — port from sibling, simplified:
   ...
   - Write `labels_mapping.json` by calling `trainer.datamodule.save_labels_mapping(Path(self.output_dir) / "labels_mapping.json")` if datamodule available. Note: `save_labels_mapping` expects a FULL FILE PATH, not a directory — it calls `open(save_path, "w")` directly.
